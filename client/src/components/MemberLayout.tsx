...
const navItems = [
  { to: "/", label: "Catalogue", end: true, icon: BookOpenIcon },
  { to: "/my-card", label: "My Card", icon: IdentificationIcon },
  { to: "/blog", label: "Blog", icon: NewspaperIcon },
  { to: "/bulletin", label: "Bulletin", icon: MegaphoneIcon },
];

export function MemberLayout({ children }: { children: ReactNode }) {
  const { name, logout } = useMemberAuth();
